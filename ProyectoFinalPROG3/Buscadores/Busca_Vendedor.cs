...
{
    public partial class Busca_Vendedor : Form
    {
        public Busca_Vendedor()
        {
            InitializeComponent();
        }
        ...
        private void Busca_Vendedor_Load(object sender, EventArgs e)
        {
        ...
        private void Consulta_Click(object sender, EventArgs e)
        {
            ...
            else if (comboVendedor.SelectedItem.Equals("No.Vendedor"))
            {
                try
                {
                    string consulta = "Select * from vendedor where ID = '" + FiltroUser.Text + "'";
                    NpgsqlDataAdapter adaptador = new NpgsqlDataAdapter(consulta, conexion);
                    DataTable dt = new DataTable();
                    adaptador.Fill(dt);
                    dataGridView1.DataSource = dt;
            ...
            else if (comboVendedor.SelectedItem.Equals("Nombre"))
            {
                try
                {

                    string consulta = "Select * from vendedor where Nombre completo = '" + FiltroUser.Text + "'";
                    NpgsqlDataAdapter adaptador = new NpgsqlDataAdapter(consulta, conexion);
                    DataTable dt = new DataTable();
                    adaptador.Fill(dt);
                    dataGridView1.DataSource = dt;
            ...
            else if (comboVendedor.SelectedItem.Equals("Apellido"))
            {
                try
                {
        ...
        private void dataGridView1_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.RowIndex >= 0)
            {
                try
                {
                    ...
                    ar.Vendedor.Text = selectedRow.Cells[1].Value.ToString();
