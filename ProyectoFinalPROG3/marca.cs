...
namespace ProyectoFinalPROG3
{
    public partial class marca : Form
    {
        ...
        private void crear()
        {
            ...
            DialogResult result = MessageBox.Show("¿Desesas crear una nueva marca?", "Confirmación", MessageBoxButtons.YesNo, MessageBoxIcon.Question);

            if (result == DialogResult.Yes)
            {
                NpgsqlCommand cmd = new NpgsqlCommand("insert into marca (\"ID\",\"Descripcion\",activo) VALUES ('" + textid.Text + "','" + textdescripcion.Text + "','" + "true" + "')", cn);
                try
                {

                    cmd.ExecuteNonQuery();
                    MessageBox.Show("Marca creada con exito", "Operacion exitosa", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                catch (Exception)
                {
                    MessageBox.Show("Error al crear marca", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
        ...
        private void actualizar()
        {
            ...
            NpgsqlConnection cn = Clases.dbconeccion.conectar();
            DialogResult result = MessageBox.Show("¿Se ha encontrado una marca, deseas actualizarla?", "Confirmación", MessageBoxButtons.YesNo, MessageBoxIcon.Question);


            if (result == DialogResult.Yes)
            {
                NpgsqlCommand cmd = new NpgsqlCommand("update marca set \"Descripcion\" ='" + textdescripcion.Text + "'where \"ID\"='" + textid.Text + "'", cn);
                try
                {

                    cmd.ExecuteNonQuery();
                    MessageBox.Show("Marca actualizada con exito", "Operacion exitosa", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                catch (Exception)
                {
                    MessageBox.Show("Error al actualizar marca", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
                finally { cn.Close(); cmd.Dispose(); }
            }
        }
        ...
        private void borrar()
        {
            ...
            else
            {
                ...
                NpgsqlCommand cmd = new NpgsqlCommand("update marca set activo = false where activo=true and \"ID\"='" + textid.Text + "'", cn);
                ...
                try
                {
                    ...
                    {
                        cmd.ExecuteNonQuery();
                        MessageBox.Show("Marca eliminada con exito", "Operacion exitosa", MessageBoxButtons.OK, MessageBoxIcon.Information);


                        cambiarcolor();

                    }
                    else { MessageBox.Show("Marca no encontrada", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error); cambiarcolor(); }
        ...
        private void Chart()
        {
            ...
            cm.CommandText = "select \"ID\",\"Descripcion\" from marca where activo=true;";
