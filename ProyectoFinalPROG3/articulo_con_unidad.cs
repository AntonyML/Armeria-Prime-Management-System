...
{
    public partial class articulo_con_unidad : Form
    {
        public articulo_con_unidad()
        {
            InitializeComponent();
            Clases.dbconeccion.MakeMovable(this);
            // Registrar en la auditoría
            Auditoria auditoria = new Auditoria();
            auditoria.RegistrarAuditoria(0, "Abrir Articulo_con_unidad"); // Reemplaza "UsuarioActual" con el ID del usuario actual
        ...
        private void articulo_con_unidad_Load(object sender, EventArgs e)
        {
            Chart();
        ...
        private void dataGridView1_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.RowIndex >= 0)
            {
                ...
                try
                {
                    ...
                    string query = "select \"ID\" from unidad where activo=true and \"ID\"=@id";
                    ...
                    using (var command = new NpgsqlCommand(query, cn))
                    {
                        ...
                        using (var reader = command.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                formbase.txtunidad.Text = reader["ID"].ToString();
