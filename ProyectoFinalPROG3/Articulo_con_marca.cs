...
{
    public partial class Articulo_con_marca : Form
    {
        public Articulo_con_marca()
        {
            InitializeComponent();
            Clases.dbconeccion.MakeMovable(this);
            // Registrar en la auditoría
            Auditoria auditoria = new Auditoria();
            auditoria.RegistrarAuditoria(0, "Abrir Articulo_con_marca"); // Reemplaza "UsuarioActual" con el ID del usuario actual
        ...
        private void articulo_con_marca_Load(object sender, EventArgs e)
        {
            Chart();
        ...
        private void dataGridView1_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.RowIndex >= 0)
            {
                ...
                try
                {
                    ...
                    string query = "select \"ID\" from marca where activo=true and \"ID\"=@id";
                    ...
                    using (var command = new NpgsqlCommand(query, cn))
                    {
                        ...
                        using (var reader = command.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                formbase.txtmarca.Text = reader["ID"].ToString();
